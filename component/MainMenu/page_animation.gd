...
var _tween_first_card : Tween

func _ready() -> void:
	bg_arr = [texture_rect ,texture_rect_2 ,texture_rect_3]
	card_arr = [card_panel,card_panel_2,card_panel_3]
	
	call_deferred("show_first_card")


func show_first_card() -> void:
	await get_tree().process_frame
	card_arr[0].scale = Vector2(0.1,0.1)
	card_arr[0].visible =true
	await get_tree().process_frame
	
	_tween_first_card = create_tween()
	_tween_first_card.set_ease(Tween.EASE_OUT)
	_tween_first_card.set_trans(Tween.TRANS_BOUNCE)  # 优雅的回弹效果
	#_tween_first_card.set_parallel()
	_tween_first_card.tween_property(card_arr[0], "scale", Vector2.ONE, 0.6)
...
func turn_to_page(current_page: int,target_page : int):
	...
	##数组应该从-1开始 这个由animation节点处理 调用该函数应该使用MainiMenu的
	
	##背景动画 默认翻下页 自动识别上下翻页动画
	...
	##产生顿挫感
	if _tween_bg_2 and _tween_bg_2.is_running():
		_tween_bg_2.kill()
	_tween_bg_2 = create_tween()
	_tween_bg_2 = create_tween()
	_tween_bg_2.set_ease(Tween.EASE_OUT)
	_tween_bg_2.set_trans(Tween.TRANS_BOUNCE)  # 优雅的回弹效果
	#_tween_bg_2.set_parallel()
	_tween_bg_2.tween_property(bg_arr[current_page], "scale", Vector2(0.8,0.8), 0.3)
	await _tween_bg_2.finished
	
	
	bg_arr[target_page].visible =true
	if _tween_bg_1 and _tween_bg_1.is_running():
		_tween_bg_1.kill()
	_tween_bg_1 = create_tween()
	_tween_bg_1.set_ease(Tween.EASE_OUT)
	#_tween_bg_1.set_trans(Tween.TRANS_SINE)  # 优雅的回弹效果
	...
	_tween_bg_2 = create_tween()
	_tween_bg_2.set_ease(Tween.EASE_OUT)
	_tween_bg_2.set_trans(Tween.TRANS_SINE)  # 优雅的回弹效果
	#_tween_bg_2.set_parallel()
	bg_arr[target_page].scale = Vector2(0.1,0.1)
	_tween_bg_2.tween_property(bg_arr[target_page], "modulate:a",0.5, 0.6)
	await _tween_bg_2.finished
	_tween_bg_2 = create_tween()
	_tween_bg_2.set_ease(Tween.EASE_OUT)
	#_tween_bg_2.set_trans(Tween.TRANS_SINE)  # 优雅的回弹效果
	...
	if _tween_card and _tween_card.is_running():
		_tween_card.kill()
	_tween_card = create_tween()
	_tween_card.set_ease(Tween.EASE_OUT)
	_tween_card.set_trans(Tween.TRANS_BOUNCE)  # 优雅的回弹效果
	#_tween_card.set_parallel()
	_tween_card.tween_property(card_arr[current_page], "scale", Vector2(0.1,0.1), 0.6)
	await _tween_card.finished
	
	card_arr[current_page].visible = false
	card_arr[target_page].visible = true
	card_arr[target_page].scale = Vector2(0.1,0.1)
	_tween_card.tween_property(card_arr[target_page], "scale", Vector2.ONE, 0.6)
	#await _tween_card.finished
	#card_arr[current_page].visible = false
	#card_arr[target_page].visible = true
