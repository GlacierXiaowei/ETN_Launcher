...
func select_zip_file() -> bool:
	var file_dialog := FileDialog.new()
	file_dialog.file_mode = FileDialog.FILE_MODE_OPEN_FILE
	file_dialog.access = FileDialog.ACCESS_FILESYSTEM
	file_dialog.add_filter("*.zip")
	...
	if selected_path.is_empty():
		return false
	if not is_valid_zip(selected_path):
		return false
...
func is_valid_zip(path: String) -> bool:
	...
	return bytes[0] == 0x50 and bytes[1] == 0x4B
